...
export default function ComponentDisplayRecords() {
  ...
  // Aggregate interest payment data from all members' loans for the selected year
  const aggregatedInterests = memberData.map(member => {
    const loans = member.records?.loans || [];
    ...
    loans.forEach(loan => {
      loan.paymentSchedule.forEach(payment => {
        if (payment.year === selectedYear) {
          const month = new Date(payment.year, payment.monthValue - 1).toLocaleString('default', { month: 'short' }).toLowerCase();
          monthlyTotals[month] += parseFloat(payment.interestPayment);
          monthlyTotals.total += parseFloat(payment.interestPayment);
        }
      });
    });
  ...
  const monthlyTotals = columns.reduce((acc, column) => {
    if (column.field !== 'fullName' && column.field !== 'total') {
      const total = aggregatedInterests.reduce((sum, row) => {
