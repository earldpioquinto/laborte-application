...
export default function ComponentDisplayRecords() {
  ...
  // Aggregate payments data from all members for the selected year
  const aggregatedPayments = memberData.map(member => {
    const payments = member.records?.payments || [];
    const paymentsForYear = payments.filter(record => record.year === selectedYear);
    ...
    paymentsForYear.forEach(record => {
  ...
  // Debugging: Log the aggregated payments
  ...
  const monthlyTotals = columns.reduce((acc, column) => {
    if (column.field !== 'fullName' && column.field !== 'total') {
      const total = aggregatedPayments.reduce((sum, row) => {
