...
export default function ComponentDisplayRecords() {
  ...
  // Aggregate loans data from all members for the selected year
  const aggregatedLoans = memberData.map(member => {
    const loans = member.records?.loans || [];
    const loansForYear = loans.filter(record => record.year === selectedYear);
  ...
  // Debugging: Log the aggregated loans
  console.log('Aggregated Loans:', aggregatedLoans);

  // Calculate monthly totals
  const monthlyTotals = columns.reduce((acc, column) => {
    if (column.field !== 'fullName' && column.field !== 'total') {
      const total = aggregatedLoans.reduce((sum, row) => {
