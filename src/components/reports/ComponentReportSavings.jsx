...
export default function ComponentDisplayRecords() {
  ...
  // Aggregate savings data from all members for the selected year
  const aggregatedSavings = memberData.map(member => {
    const savings = member.records?.savings || [];
    const savingsForYear = savings.filter(record => record.year === selectedYear);
  ...
  // Debugging: Log the aggregated savings
  console.log('Aggregated Savings:', aggregatedSavings);

  // Calculate monthly totals
  const monthlyTotals = columns.reduce((acc, column) => {
    if (column.field !== 'fullName' && column.field !== 'total') {
      const total = aggregatedSavings.reduce((sum, row) => {
